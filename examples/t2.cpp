...
static_trait(Compare, (Self), (
  (type, value_type),
  hof(bool, less_than, (Self, Self, fn(bool, value_type, value_type))),
))
...
template <typename T>
struct Compare::Impl<Box<T>> {
  using value_type = T;

  template <class F>
  static bool less_than(Box<T> a, Box<T> b, F &&fn) {
    return static_cast<bool>(std::invoke(std::forward<F>(fn), a.inner, b.inner));
  }
};

template <>
struct Test::Impl<int> {
  using Factor = bool;
...
static_assert(std::same_as<decltype(Functor::map(Box<int>{41}, [](auto x) { return std::to_string(x); })), Box<std::string>>);
static_assert(std::same_as<decltype(Zip::zip_with(Box<int>{2}, Box<int>{3}, [](auto a, auto b) { return a + b; })), Box<int>>);
static_assert(std::same_as<decltype(Compare::less_than(Box<int>{2}, Box<int>{3}, is_less)), bool>);
...
int main() {
  ...
  auto z = Zip::zip_with(Box<int>{2}, Box<int>{3}, [](auto a, auto b) {
    return a + b;
  });
  std::cout << z.inner << "\n";

  std::cout << std::boolalpha << Compare::less_than(Box<int>{2}, Box<int>{3}, is_less) << "\n";
